...
// Functions can access any variables outside of its scope even when the variable is in a function that its inside
// In this case its called a closure
...
function animalNames(arr) {
  ...
  arr.forEach((val) =>
    displayNames.push(
      `name: ${val.animal_name}, scientific: ${val.scientific_name}`
    )
  );
...
function lowerCaseNames(arr) {
  let lowerCase = arr.map((obj) => obj.animal_name.toLowerCase());
  return lowerCase;
}
...
function lowPopulationAnimals(arr) {
  let filter = arr.filter((obj) => obj.population < 5);
  return filter;
}
...
CuboidMaker.prototype.surfaceArea = function () {
  ...
  let y = this.length * this.height;
  let z = this.width * this.height;
  let sum = (x + y + z) * 2;
  return sum;
};
